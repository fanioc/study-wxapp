...
function init(that, barHeight, keys, isShowKey, isShowHis, callBack) {
  ...
  temData.keys = keys;
  //lly
  let tag_group_choose = [];
  for (let i in keys) {
    tag_group_choose.push(false);
  }
  temData.tag_group_choose = tag_group_choose;
  wx.getSystemInfo({
...
function wxSearchKeyTap(e, that, callBack) {
  ...
  let index = temData.keys.indexOf(e.target.dataset.key);
  //console.log('temData.keys.indexOf(e.target.dataset.key)',index);lly
